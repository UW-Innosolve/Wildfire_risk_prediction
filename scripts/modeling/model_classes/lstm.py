...
class LSTM_3D(BaseModel):
    ...
    def forward(self, x, eval=False):
        """
        Args:
            x: Input tensor of shape [batch_size, 1, 38, 37, 34]
        Returns:
            output: Output tensor of shape [batch_size, 37, 34]
        """
        if eval:
            batch_size, time_steps, depth, height, width = (1, 14, 38, 37, 34)
        else:
            batch_size, time_steps, depth, height, width = (10, 14, 38, 37, 34)
