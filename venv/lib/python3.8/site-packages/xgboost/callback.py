...
class EarlyStopping(TrainingCallback):
    ...
    data_name :
        Name of parameters that is used for early stopping.
    maximize :
    ...
    def after_iteration(
        ...
        msg = "Must have at least 1 validation parameters for early stopping."
        ...
        if data_name not in evals_log:
            raise ValueError(f"No parameters named: {data_name}")

        if not isinstance(data_name, str):
            raise TypeError(
                f"The name of the parameters should be a string. Got: {type(data_name)}"
            )
