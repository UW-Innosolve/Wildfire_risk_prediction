...
class _SparkXGBParams(
    ...
    force_repartition = Param(
        ...
        + "want to force the input parameters to be repartitioned before XGBoost training."
    ...
    repartition_random_shuffle = Param(
        ...
        "parameters when repartitioning is required. By default is True.",
        TypeConverters.toBoolean,
    )
    feature_names = Param(
        Params._dummy(),
...
class _SparkXGBEstimator(Estimator, _SparkXGBParams, MLReadable, MLWritable):
    ...
    def _repartition_needed(self, dataset: DataFrame) -> bool:
        """
        We repartition the parameters if the number of workers is not equal to the number of
        partitions."""
    ...
    def _prepare_input(self, dataset: DataFrame) -> Tuple[DataFrame, FeatureProp]:
        ...
        if self._repartition_needed(dataset):
            ...
            # parameters, users must set force_repartition to true to force repartition.
            # Or else some partitions might contain only train or validation parameters.
...
class _SparkXGBModel(Model, _SparkXGBParams, MLReadable, MLWritable):
    ...
    def _get_feature_col(
        ...
        if feature_col_names and set(feature_col_names).issubset(set(dataset.columns)):
            # The model is trained with features_cols and the predicted parameters
            ...
            # 1. The model was trained by features_cols, but the parameters doesn't contain
            #       all the columns specified by features_cols, so we need to check if
            #       the dataframe has the featuresCol
            # 2. The model was trained by featuresCol, and the predicted parameters must contain
            #       featuresCol column.
            feature_col_names = None
            features_col.append(
