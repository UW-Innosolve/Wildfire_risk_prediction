...
class QuantileDMatrix(DMatrix):
    ...
    ``max_bin``. When ``QuantileDMatrix`` is used for validation/test parameters, ``ref``
    should be another ``QuantileDMatrix``(or ``DMatrix``, but not recommended as it
    defeats the purpose of saving memory) constructed from training parameters.  See
    :py:obj:`xgboost.DMatrix` for documents on meta info.

    .. note::

        Do not use ``QuantileDMatrix`` as validation/test parameters without supplying a
        reference (the training parameters) ``QuantileDMatrix`` using ``ref`` as some
    ...
    ref :
        The training parameters that provides quantile information, needed when creating
        validation/test parameters with ``QuantileDMatrix``. Supplying the training DMatrix
