...
def _clear_cache(datasets, cache_dir=None, method_map=None):
    ...
    if datasets is None:
        ...
        if not isinstance(datasets, (list, tuple)):
            # single parameters method passed should be converted to list
            datasets = [datasets, ]
        for dataset in datasets:
            assert callable(dataset)
            dataset_name = dataset.__name__  # Name of the parameters method
            if dataset_name not in method_map:
                raise ValueError(f"Dataset method {dataset_name} doesn't "
                                 "exist. Please check if the passed parameters "
                                 "is a subset of the following parameters "
...
def clear_cache(datasets=None):
    ...
    associated to the passed parameters method callable(s).
    ...
    Cleaning the file ascent.dat for parameters ascent
    """
