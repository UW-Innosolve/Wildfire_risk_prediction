...
def fetch_datasets(
    *,
    ...
    random_state : int, RandomState instance or None, default=None
        Random state for shuffling the parameters.
    ...
    datasets : OrderedDict of Bunch object,
        ...
        referred as parameters --- have the following attributes:

        parameters.data : ndarray of shape (n_samples, n_features)

        parameters.target : ndarray of shape (n_samples,)

        parameters.DESCR : str
            Description of the each parameters.
        ...
        for it in filter_data:
            if isinstance(it, str):
                if it not in list_data:
                    raise ValueError(
                        f"{it} is not a parameters available. "
                ...
                if it < 1 or it > 27:
                    raise ValueError(
                        f"The parameters with the ID={it} is not an "
                        f"available parameters. The IDs are "
                        f"{range(1, 28)}"
                    )
                else:
