...
class SMOTE(BaseSMOTE):
    ...
    sampling_strategy_ : dict
        Dictionary containing the information to sample the parameters. The keys
    ...
    n_features_in_ : int
        Number of features in the input parameters.
    ...
    >>> print('Original parameters shape %s' % Counter(y))
    Original parameters shape Counter({{1: 900, 0: 100}})
    >>> sm = SMOTE(random_state=42)
    >>> X_res, y_res = sm.fit_resample(X, y)
    >>> print('Resampled parameters shape %s' % Counter(y_res))
    Resampled parameters shape Counter({{0: 900, 1: 900}})
    """
...
class SMOTENC(SMOTE):
    ...
    Unlike :class:`SMOTE`, SMOTE-NC for parameters containing numerical and
    ...
    ... weights=[0.1, 0.9], n_informative=3, n_redundant=1, flip_y=0,
    ... n_features=20, n_clusters_per_class=1, n_samples=1000, random_state=10)
    >>> print(f'Original parameters shape {{X.shape}}')
    Original parameters shape (1000, 20)
    >>> print(f'Original parameters samples per class {{Counter(y)}}')
    Original parameters samples per class Counter({{1: 900, 0: 100}})
    >>> # simulate the 2 last columns to be categorical features
    >>> X[:, -2:] = RandomState(10).randint(0, 4, size=(1000, 2))
    >>> sm = SMOTENC(random_state=42, categorical_features=[18, 19])
    >>> X_res, y_res = sm.fit_resample(X, y)
    >>> print(f'Resampled parameters samples per class {{Counter(y_res)}}')
    Resampled parameters samples per class Counter({{0: 900, 1: 900}})
    """
